import { Word, WordCategory, words as defaultWords } from './words-data-en';
import { supabase } from './supabase';

const STORAGE_KEY = 'english-words-v2';
const PROGRESS_KEY = 'english-progress';
const DIFFICULT_WORDS_KEY = 'english-difficult';
const SRS_KEY = 'english-srs';
...
export const exportWordsToExcel = async (category?: WordCategory): Promise<Blob> => {
  ...
  const data = [
    ['Английский', 'Русский', 'Транскрипция', 'Категория'],
    ...words.map(w => [w.english, w.russian, w.transcription, w.category])
  ];
...
export const getProgressFromCloud = async (): Promise<AppProgress> => {
  ...
  try {
    ...
    console.error('Error fetching EN progress from cloud:', e);
    return getProgress();
  }
};
...
export const saveLessonProgress = async (category: WordCategory, lessonIndex: number, score: number, total: number): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      await supabase.from('user_progress_en').upsert({
      ...
      console.error('Error saving EN progress to cloud:', e);
    }
  }
};
...
export const resetProgress = async (): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      await supabase.from('user_progress_en').delete().eq('user_id', currentUserId);
    } catch (e) {
      console.error('Error resetting EN progress in cloud:', e);
    }
  }
};
...
export const getDifficultWordsFromCloud = async (): Promise<DifficultWord[]> => {
  ...
  try {
    ...
    console.error('Error fetching EN difficult words from cloud:', e);
...
export const markWordAsDifficult = async (wordId: string): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      ...
      await supabase.from('user_difficult_words_en').upsert({
        user_id: currentUserId,
        word_id: wordId,
      ...
      console.error('Error saving EN difficult word to cloud:', e);
    }
  }
};
...
export const removeDifficultWord = async (wordId: string): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      await supabase.from('user_difficult_words_en')
        .delete()
        .eq('user_id', currentUserId)
        .eq('word_id', wordId);
    } catch (e) {
      console.error('Error removing EN difficult word from cloud:', e);
    }
  }
};
...
export const clearDifficultWords = async (): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      await supabase.from('user_difficult_words_en').delete().eq('user_id', currentUserId);
    } catch (e) {
      console.error('Error clearing EN difficult words in cloud:', e);
    }
  }
};
...
export const getSRSDataFromCloud = async (): Promise<SRSData[]> => {
  ...
  try {
    ...
    console.error('Error fetching EN SRS data from cloud:', e);
    return getSRSData();
  }
};
...
export const updateSRS = async (wordId: string, quality: number): Promise<void> => {
  ...
  if (currentUserId) {
    try {
      await supabase.from('user_srs_en').upsert({
        user_id: currentUserId,
        word_id: wordId,
      ...
      console.error('Error saving EN SRS data to cloud:', e);
    }
  }
};
